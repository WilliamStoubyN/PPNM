...
std::tuple<vector, vector> rkStep12(std::function<vector(double, vector)> F, double h, double x, vector y) {
    vector k0 = f(x,y);
...
std::tuple<vector, std::vector<vector>> rkDriver(std::function<vector(double, vector)> F, double a, double b, vector yInit, double h0, double acc, double eps) {
    ...
    while(true) {
        ...
        if(err > 0) {h *= std::min(std::pow(tol/error, 0.25) * 0.95, 2);} //readjust the step-size
        else {h *= 2;}
    }
}